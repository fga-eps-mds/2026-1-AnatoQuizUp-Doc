...
> 📌 Este documento descreve o **Modelo V1** (conceitual e lógico inicial, focado no Auth DB). Para o modelo de dados **atual** — com os dois bancos e todos os models (amizades, moedas, listas, turmas e dashboards) — veja [Modelo Atual](modelo-atual.md).
...
> **Nota de arquitetura (2026-05-13):** este documento descreve o **Auth DB**, pertencente ao `2026-1-AnatoQuizUp-Usuario-Service`. A lógica de quiz foi fragmentada para o `2026-1-AnatoQuizUp-Quiz-Service`, que possui **Quiz DB** próprio com `Tema`, `Questao`, `QuestaoAlternativa` e `ResolucaoQuestao`. Não há FK entre Quiz DB e Auth DB; IDs de usuário no Quiz-Service são referências externas.
...
| 13/05/2026 | 1.1 | Nota de arquitetura indicando separacao entre Auth DB e Quiz DB | Miguel Moreira |
