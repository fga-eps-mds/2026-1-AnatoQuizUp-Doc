...
- **Escopo/cronograma (SPI ≈ 0,72–0,85, terminando em 0,81):** o time entregou **~81% do que planejou** acumuladamente. Esse número subiu de ~0,66 para 0,81 depois que a equipe **quitou um débito de estimativa** (ver abaixo): havia ~36 SP de trabalho realmente entregue (turmas, recompensas, listas, dashboards de aluno/turma, amizades) que estava sem `estimate` no board e por isso não contava no EV. O burnup ainda mostra PV acima de EV — houve planejamento acima da capacidade (S1 planejou 91 SP; velocity mediana ~16 SP/sprint) — mas a entrega real ficou mais perto do plano do que o registro inicial sugeria.
...
| **D7** (R3) | 73% das issues sem estimate; SPI de S6–S8 colapsava por falta de medição, não (só) de entrega | Sem estimates, EVM e velocity perdem validade — o dashboard fica cego | **Executado:** equipe pontuou os cards entregues sem estimate (sizing retrospectivo); IA aplicou no ZenHub. **Adiante:** estimate obrigatório ao puxar para Sprint Backlog | SPI corrigido de 0,66 → **0,81**; S5 27→48 SP, S7 0→15 SP, com a estimativa real do time (não auto-preenchimento) |"""))
...
- **O modelo de EVM passou por iterações até ficar fiel ao exemplo da disciplina.** Uma versão intermediária definiu o cronograma por `PPC = sprints decorridas / 10` (tempo) enquanto o escopo da S1 sozinho era ~38% do total planejado — isso fazia o SPI **explodir nas primeiras sprints** (S1 = 7,8) e o gráfico cortava esses valores. O time rejeitou: o enquadramento do anexo do professor é em story points (`SPI = SP entregues / SP planejados`), o que dá uma série estável (SPI termina em 0,81). Exemplo concreto de por que output de IA não se aceita sem verificar contra a fonte (aqui, o próprio exemplo do professor).
...
O semestre fecha com **produto saudável** (Qualidade do Produto 0,86; 0 bugs abertos no SonarCloud; cobertura ponderada ~90%), **processo com fluxo curto e estável** (cycle time ~2 dias; WIP dentro dos limites da Lei de Little) e **projeto com lição clara de higiene de dados**: o EVM ágil mostra entrega de ~81% do planejado (SPI ≈ 0,81) a um custo levemente acima do orçado (CPI ≈ 0,80, VAC ≈ −R$ 2,3 mil). O número só ficou fiel depois de quitar o débito de estimativa — o board mal mantido escondia ~36 SP de entrega real. A resposta gerencial (D6/D7) é planejar a R3 final dentro da capacidade medida (~16 SP/sprint) **e** manter a disciplina de estimar e fechar cards na hora, para o EVM não voltar a mentir contra o próprio time.
